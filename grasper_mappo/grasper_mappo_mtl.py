...
from grasper_mappo.runner_shared.env_runner import EnvRunner as Runner
from grasper_mappo.config import get_config, get_train_utility_results_dir, get_mtl_model_results_dir, get_runs_dir
...
def grasper_mappo_mtl(args):
    ...
    setup_str = f"End2End: {args.use_end_to_end}, Emb Layer: {args.use_emb_layer}, Augment: {args.use_augmentation}, Act Supervisor: {args.use_act_supervisor}"
    ...
    if args.use_end_to_end:
        ...
        hgs = [hg for _ in range(game._defender_num)]
    else:
    ...
    if args.checkpoint > 0:
        ...
        if os.path.exists(result_pth) and os.path.getsize(result_pth) > 0:
            ...
            reward_list, time_list, aloss_list, vloss_list, itera_list = data['reward_list'], data['time_list'], \
                data['aloss_list'], data['vloss_list'], data['itera_list']
    ...
    for iteration in range(start_iter, args.num_iterations):
        ...
        if args.use_end_to_end:
            episode_reward = runner.run_one_episode_e2e(hgs, Ts, use_act_supervisor=args.use_act_supervisor)
        else:
